...
try { db.exec('ALTER TABLE grades ADD COLUMN lesson INTEGER'); } catch {}
...
function getGrades(userId) {
  return db.prepare(`SELECT id, subject, grade, activity_type AS activityType, weight,
    grade_date AS date, lesson, homework_date AS homeworkDate, homework_lesson AS homeworkLesson, created_at AS createdAt
...
function addGrade(userId, input) {
  ...
  const lesson = Number.isInteger(Number(input.lesson)) ? Number(input.lesson) : null;
  ...
  const result = db.prepare(`INSERT INTO grades (user_id, subject, grade, activity_type, weight, grade_date, lesson, homework_date, homework_lesson, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`)
    .run(userId, subject, grade, activityType, weight, date, lesson, homeworkDate, homeworkLesson, new Date().toISOString());
  return db.prepare(`SELECT id, subject, grade, activity_type AS activityType, weight,
    grade_date AS date, lesson, homework_date AS homeworkDate, homework_lesson AS homeworkLesson, created_at AS createdAt
